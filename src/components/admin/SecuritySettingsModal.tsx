    ...
    try {
      ...
      const verifyRes = await fetch('/api/auth/2fa', {
        ...
        body: JSON.stringify({
          ...
          clientDataJSON,
          deviceName: passkeyDeviceName.trim() || 'MacBook Touch ID / Biometric Device',
        }),
      });
            ...
            <div className="p-4 rounded-2xl bg-black/40 border border-white/10 space-y-3">
              ...
              <div className="flex flex-col sm:flex-row gap-2 pt-2">
                <input
                  ...
                  className="flex-1 px-3.5 py-2.5 rounded-xl bg-black/60 border border-white/10 text-white text-xs focus:outline-none focus:border-red-500"
                />
                <button
                  type="button"
                  onClick={handleRegisterPasskey}
                  disabled={isRegisteringPasskey}
                  className="px-4 py-2.5 rounded-xl text-xs font-bold bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 text-white shadow-lg shadow-red-600/30 transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 shrink-0"
                >
